...
jQuery( document ).ready( function( $ ) {
	...
	woofc_icon_show();
	$( '#woofc_count_icon' ).on( 'change', function() {
		woofc_icon_show();
	} );

	// choose background image
	var woofc_file_frame;
	$( '#woofc_upload_image_button' ).on( 'click', function( event ) {
		...
		woofc_file_frame.on( 'select', function() {
			...
			var attachment = woofc_file_frame.state().get( 'selection' ).first().toJSON();

			// Do something with attachment.id and/or attachment.url here
			if ( $( '#woofc_image_preview img' ).length ) {
				$( '#woofc_image_preview img' ).attr( 'src', attachment.url );
			} else {
				$( '#woofc_image_preview' ).html( '<img src="' + attachment.url + '"/>' );
			}

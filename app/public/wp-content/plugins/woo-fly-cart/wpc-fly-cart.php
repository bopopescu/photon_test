...
! defined( 'WOOFC_VERSION' ) && define( 'WOOFC_VERSION', '2.0.7' );
		...
		if ( ! class_exists( 'WPCleverWoofc' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoofc {
				...
				function woofc_row_meta( $links, $file ) {
					...
					if ( $plugin === $file ) {
						$row_meta = array(
							'support' => '<a href="https://wpclever.net/support?utm_source=support&utm_medium=woofc&utm_campaign=wporg" target="_blank">' . esc_html__( 'Premium support', 'woofc' ) . '</a>',
						);
                                    ...
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <span class="description"><?php esc_html_e( 'Show the close button on the fly cart?', 'woofc' ); ?></span>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            <th><?php esc_html_e( 'Product data', 'woofc' ); ?></th>
                                            <td>
                                                ...
                                                <span class="description">
											<?php esc_html_e( 'Show the product data under product title on the list?', 'woofc' ); ?>
										</span>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
											...
											<?php esc_html_e( 'Show the cross sells products?', 'woofc' ); ?>
										</span>
                                                <p style="color: red">
                                                    This feature is just available for Premium Version. Click <a
                                                            href="https://wpclever.net/downloads/woocommerce-fly-cart?utm_source=pro&utm_medium=woofc&utm_campaign=wporg"
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <span class="description">
											<?php esc_html_e( 'Show the continue shopping button at the end of fly cart?', 'woofc' ); ?>
										</span>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <span class="description">
											<?php esc_html_e( 'Enable undo after removing a product?', 'woofc' ); ?>
										</span>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr class="submit">
                                            <th colspan="2">
                                                ...
                                                <input type="hidden" name="page_options"
                                                       value="_woofc_auto_show_ajax,_woofc_auto_show_normal,_woofc_manual_show,_woofc_overlay_layer,_woofc_position,_woofc_style,_woofc_color,_woofc_bg_image,_woofc_cart_heading,_woofc_close,_woofc_link,_woofc_data,_woofc_subtotal,_woofc_coupon,_woofc_total,_woofc_buttons,_woofc_cross_sells,_woofc_continue,_woofc_continue_url,_woofc_undo_remove,_woofc_reload,_woofc_hide_cart_checkout,_woofc_menus,_woofc_count,_woofc_count_position,_woofc_count_icon,_woofc_count_hide_empty"/>
                                            </th>
                                        </tr>
                                    </table>
                                </form>
				...
				function woofc_get_cart_area() {
					...
					$cart_html = '<div id="woofc-area" class="woofc-area woofc-effect-' . esc_attr( get_option( '_woofc_position', '05' ) ) . ' woofc-style-' . esc_attr( get_option( '_woofc_style', '01' ) ) . '">';
					...
					if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
						...
						foreach ( $items as $cart_item_key => $cart_item ) {
							if ( ! isset( $cart_item['bundled_by'] ) && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
								$woofc_product      = apply_filters( 'woocommerce_cart_item_product', $cart_item['data'], $cart_item, $cart_item_key );
								$woofc_product_id   = apply_filters( 'woocommerce_cart_item_product_id', $cart_item['product_id'], $cart_item, $cart_item_key );
								$woofc_link         = get_option( '_woofc_link', 'yes' );
								$woofc_product_link = apply_filters( 'woocommerce_cart_item_permalink', $woofc_product->is_visible() ? $woofc_product->get_permalink( $cart_item ) : '', $cart_item, $cart_item_key );
								$cart_html          .= '<div class="' . esc_attr( apply_filters( 'woocommerce_cart_item_class', 'woofc-item', $cart_item, $cart_item_key ) ) . '" data-key="' . esc_attr( $cart_item_key ) . '" data-name="' . esc_attr( $woofc_product->get_name() ) . '"><div class="woofc-item-inner">';
								$cart_html          .= '<div class="woofc-item-thumb">';

								if ( ( $woofc_link !== 'no' ) && ! empty( $woofc_product_link ) ) {
									$cart_item_thumbnail = sprintf( '<a ' . ( $woofc_link === 'yes_popup' ? 'class="woosq-btn" data-id="' . $woofc_product_id . '"' : '' ) . ' href="%s" ' . ( $woofc_link === 'yes_blank' ? 'target="_blank"' : '' ) . '>%s</a>', esc_url( $woofc_product_link ), $woofc_product->get_image() );
								} else {
									$cart_item_thumbnail = $woofc_product->get_image();
								}
								...
								if ( ( $woofc_link !== 'no' ) && ! empty( $woofc_product_link ) ) {
									$cart_item_name = sprintf( '<a ' . ( $woofc_link === 'yes_popup' ? 'class="woosq-btn" data-id="' . $woofc_product_id . '"' : '' ) . ' href="%s" ' . ( $woofc_link === 'yes_blank' ? 'target="_blank"' : '' ) . '>%s</a>', esc_url( $woofc_product_link ), $woofc_product->get_name() );
								} else {
									$cart_item_name = $woofc_product->get_name();
								}
								...
								if ( get_option( '_woofc_data', 'no' ) === 'yes' ) {
									$cart_html .= '<span class="woofc-item-data">' . wc_get_formatted_cart_item_data( $cart_item ) . '</span>';
								}

								$cart_html .= '<span class="woofc-item-price">' . apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $woofc_product ), $cart_item, $cart_item_key ) . '</span>';
								$cart_html .= '</div>';

								if ( $woofc_product->is_sold_individually() || ! empty( $cart_item['woosb_parent_id'] ) || ! empty( $cart_item['wooco_parent_id'] ) ) {
								...
								$cart_html        .= '<div class="woofc-item-qty"><div class="woofc-item-qty-inner">' . apply_filters( 'woocommerce_cart_item_quantity', $cart_item_quantity, $cart_item_key, $cart_item ) . '</div></div>';
